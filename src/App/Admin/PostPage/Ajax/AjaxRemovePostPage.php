                            ...
                            jQuery.post( ajaxurl, data, function( response ){
                                ...
                                if( response && response.data  && response.data.result ){
                                    if(response.data.result  === 'success' ){
                                        alexRemovePostsSubmit.after(' <span style=\'padding-left: 10px;color: green;\'> Посты удалены успешно</span>')
                                    }else{
                                        alexRemovePostsSubmit.after(` <span  style=\'padding-left: 10px;color:red;\'> Произошла неизвестная ошибка. <br/>
                                           ${response.data.result} </span>`);
                                    }
                                }else{
                                    alexRemovePostsSubmit.after(` <span  style=\'padding-left: 10px;color: red;\'> Ошибка удаления.  </span>`);
                                }
